...
generated_target = scaler2.transform(y_train)
...
hidden1 = tf.keras.layers.Dense(units=50, input_shape=[3], activation = 'relu')
hidden2 = tf.keras.layers.Dense(units=50, activation = 'relu')
...
regressormodel = models.fit(training_data['input'],training_data['targets'],epochs=1000,verbose=False)

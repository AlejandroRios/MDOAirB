...
def network_optimization(arrivals, departures, distances, demand, active_airports, doc0, pax_capacity, vehicle):
    ...
    #         for j in range(len(departures)):
    #             if j>i:
    #                 list_of_airplanes_processed_mat[i][j] = list_of_airplanes_processed[idx]
    #                 idx = idx+1
    # while idx<list_size:
    #     for i in range(len(departures)):
    #         for j in range(len(departures)):
    ...
    # print(list_of_airplanes_processed_mat)
    ...
    # kpi_df1 = pd.DataFrame.from_dict(xijk, orient="idx", 
    #                             columns = ["variable_object"])
    # kpi_df1.idx =  pd.MultiIndex.from_tuples(kpi_df1.idx, 

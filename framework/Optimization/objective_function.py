...
def objective_function(x, original_vehicle, computation_mode, route_computation_mode, airports, distances, demands):
    ...
    try:
        ...
        if status == 0:
            ...
            try:
                for i in range(len(airports)):
                    ...
                    for k in range(len(airports)):
                        ...
                        total_mission_flight_time[airports_keys[i]][airports_keys[k]]  = 0
                        mach[airports_keys[i]][airports_keys[k]]  = 0
                        passenger_capacity[airports_keys[i]][airports_keys[k]]  = 0
                        ...
                        if (i != k) and (distances[airports_keys[i]][airports_keys[k]] <= performance['range']) and (demands[airports_keys[i]][airports_keys[k]]['demand'] > 0):
                            ...
                            takeoff_runway = airports[airports_keys[i]]['runways'][demands[airports_keys[i]][airports_keys[k]]['takeoff_runway']]
                            ...
                            landing_runway = airports[airports_keys[k]]['runways'][demands[airports_keys[i]][airports_keys[k]]['landing_runway']]

                            # Calculate DOC and mission parameters for origin-destination airports pair:                        
                            mission_range = distances[airports_keys[i]][airports_keys[k]]
                            fuel_mass[airports_keys[i]][airports_keys[k]], total_mission_flight_time[airports_keys[i]][airports_keys[k]], DOC,mach[airports_keys[i]][airports_keys[k]],passenger_capacity[airports_keys[i]][airports_keys[k]], SAR[airports_keys[i]][airports_keys[k]] = mission(vehicle, airport_departure, takeoff_runway, airport_destination, landing_runway, mission_range)
                            DOC_nd[airports_keys[i]][airports_keys[k]] = DOC
                            DOC_ik[airports_keys[i]][airports_keys[k]] = int(DOC*mission_range)                       
            ...
            try:
                write_optimal_results(list(airports.keys()), distances, demands, profit, DOC_ik, vehicle, kpi_df2, airplanes_ik)
            except:
...
def main(argv):
	...
	x = [130,  91, 38, 29,  -4.5,   33, 62, 17, 30, 1480, 18, 144, 6, 3000, 41000, 78, 1, 1, 1, 1]
